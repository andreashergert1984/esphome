...
from .. import VAILLANT_X6_COMPONENT_SCHEMA, CONF_VAILLANT_X6_ID, vaillant_x6_ns

VaillantX6Select = vaillant_x6_ns.class_(
    "VaillantX6Select", select.Select, cg.PollingComponent
)

#from .. import template_ns
CONF_BURNER_LOCK_TIME = "burner_lock_time"

COMMANDS = {
    CONF_BURNER_LOCK_TIME: {
        'command': 0x59,
        'data_type': 1,
        'response_length' : 4,
        'data_length': 1,
...
TYPES = {
    CONF_BURNER_LOCK_TIME: select.SELECT_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(VaillantX6Select),
...
async def to_code(config):
    ...
    for type, _ in TYPES.items():
        if type in config:
            conf = config[type]
            # selec = await select.new_select(conf)
            selec = cg.new_Pvariable(conf[CONF_ID])
            await cg.register_component(selec, conf)
            await select.register_select(selec, conf, options=conf[CONF_OPTIONS])

            cg.add(paren.addSelect(selec,COMMANDS[type]['command'],COMMANDS[type]['data_type'],COMMANDS[type]['response_length'],COMMANDS[type]['data_length'],COMMANDS[type]['has_status'],type))
